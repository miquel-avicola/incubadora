...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const NavContent = () => (
    <div className="flex flex-col h-full">
      <div className="p-6">
        ...
        <p className="text-sm text-text-dim mono uppercase tracking-wider">{isCarrega ? `Càrrega #${carregaId}` : 'Incubadora'}</p>
      </div>
            ...
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 ${
                active 
                  ...
                  : 'text-text hover:bg-slate-100 hover:text-slate-900'
              }`}
            >
  ...
  return (
    <div className="flex h-screen bg-bg overflow-hidden text-text font-sans">
      ...
      {/* Sidebar Desktop */}
      <aside className="hidden md:block w-64 bg-surface border-r border-border shrink-0 shadow-sm z-10">
        <NavContent />
      </aside>

      {/* Mobile Top Bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 bg-surface border-b border-border z-20 flex items-center justify-between px-4 shadow-sm">
        <div className="font-bold text-text truncate">
          {isCarrega ? `Càrrega #${carregaId}` : 'Miquel Avícola'}
        </div>
        <button 
      ...
      {/* Main Content Area */}
      <main className="flex-1 relative overflow-y-auto pt-14 md:pt-0 pb-safe">
