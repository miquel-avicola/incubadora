...
export default function DetallCarregaClient({ initialFull, clients }: Props) {
  ...
  const carregarDades = useCallback(() => {
    fetch(`/api/carrega/${full.id}`)
      .then(r => r.json())
      .then(data => setFull(data))
  }, [full.id])

  async function afegirComanda() {
    if (!nouClientId || !nouQuantitat) { setErrorComanda('Omple tots els camps'); return }
  ...
  const toggleMaquilaGrup = async (lotId: number, incubadoraId: number, actualMaquila: boolean) => {
    ...
    setDesantMaquila(true)
    const res = await fetch(`/api/carrega/${full.id}/maquila-grup`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const desarPrevisioGrup = async (lotId: number, incubadoraId: number, valor: string) => {
    ...
    const res = await fetch(`/api/carrega/${full.id}/previsio-grup`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
    })
