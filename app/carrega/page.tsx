    ...
    <main style={{ background: 'var(--bg)', minHeight: '100vh', padding: '1.5rem' }}>
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {fulls.map(full => {
            const totalPollets = full.comandes
              .filter(c => c.tipus === 'Pollets')
              .reduce((s, c) => s + (c.quantitat_pollets || 0), 0)
            const totalMaquila = full.comandes
              .filter(c => c.tipus === 'Maquila')
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.4rem' }}>
                        <span style={{ fontFamily: 'IBM Plex Mono', fontWeight: 700, fontSize: '1rem', color: 'var(--accent)' }}>
                          #{full.num_carrega}
                        </span>
                        <span style={{ fontSize: '0.75rem', color: ESTAT_COLOR[full.estat] || 'var(--text-dim)', fontFamily: 'IBM Plex Mono' }}>
                          {full.estat}
                        </span>
                      </div>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      {totalPollets > 0 && (
                        <div style={{ fontSize: '0.8rem', color: 'var(--text-dim)', fontFamily: 'IBM Plex Mono' }}>
                          {totalPollets.toLocaleString()} pollets
                        </div>
                      )}
                      {totalMaquila > 0 && (
                        <div style={{ fontSize: '0.8rem', color: 'var(--text-dim)', fontFamily: 'IBM Plex Mono' }}>
                          {totalMaquila.toLocaleString()} ous maq.
                        </div>
                      )}
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)', marginTop: '0.25rem' }}>
                        {full.comandes.length} comanda{full.comandes.length !== 1 ? 's' : ''}
                      </div>
                    </div>
                  </div>
